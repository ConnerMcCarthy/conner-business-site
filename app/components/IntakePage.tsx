  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (Boolean(data.done)) {
        ...
        const leadForEmail: Record<string, unknown> = { ...mergedLead };
        if (data.pricing_estimate) {
          leadForEmail.pricing_estimate = data.pricing_estimate;
        }
        const result = await submitIntakeLead(summary, leadForEmail);
        if (result.success) {
          setEmailStatus("sent");
        } else {
